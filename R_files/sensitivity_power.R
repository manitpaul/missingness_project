# Sensitivity power-style analysis at eta = 0.5
# --------------------------------------------
# This script computes and plots mean p-values for:
#   - Proposed test
#   - Matching test
...
summarize_gamma_pvalues <- function(trial_df, conf_level = 0.90) {
  agg <- stats::aggregate(
...
run_gamma_sensitivity_power <- function(
  gamma_grid = c(1, 1.1, 1.2, 1.3, 1.4, 1.5),
  eta = 0.5,
  n_rep = 200,
  N = 2000,
  alpha = 0.05,
  ...
  show_progress = TRUE,
  save_path = "R_files/sensitivity_power_results.rds"
) {
  ...
  run_task <- function(task_id) {
    ...
    one <- run_one_sensitivity_power_experiment(
      Gamma = Gamma,
      eta = eta,
      N = N,
      alpha = alpha,
  ...
  if (.Platform$OS.type == "unix" && n_cores > 1L) {
    ...
    parallel::clusterExport(
      ...
      varlist = c(
        ...
        "K", "score", "huber_c", "run_one_sensitivity_power_experiment"
      ),
      envir = environment()
    )
    for (start_idx in seq.int(1L, n_tasks, by = batch_size)) {
  ...
  summary_df <- summarize_gamma_pvalues(trial_df, conf_level = 0.90)
...
plot_sensitivity_power <- function(
  results_obj,
  ...
  if (is.list(results_obj) && !is.null(results_obj$summary)) {
    ...
    if (show_ci_band && (!all(c("lower_ci", "upper_ci") %in% names(summary_df))) && !is.null(results_obj$trials)) {
      summary_df <- summarize_gamma_pvalues(results_obj$trials, conf_level = 0.90)
    }
  } else {
...
run_sensitivity_power_study <- function(
  gamma_grid = c(1, 1.1, 1.2, 1.3, 1.4, 1.5),
  eta = 0.5,
  n_rep = 200,
  N = 2000,
  ...
  base_seed = 12345,
  n_cores = detect_available_cores(),
  save_results_path = "R_files/sensitivity_power_results.rds",
  save_plot_path = "PDF_images/sensitivity_power.pdf"
) {
  results <- run_gamma_sensitivity_power(
    gamma_grid = gamma_grid,
    eta = eta,
    n_rep = n_rep,
    N = N,
